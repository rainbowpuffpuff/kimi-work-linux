...
# NOTE: Moonshot's redirect endpoint currently serves a single universal/arm64
# DMG. On x86_64 we still ship the arm64 asar (it is pure JS + natives we
# replace) and pair it with an x64 Linux Electron — only the runtime is arch-
# specific, the asar is not.
dmg_arch_suffix() {
	case "$(detect_arch)" in
		x64)   echo "x64" ;;
		arm64) echo "arm64" ;;
