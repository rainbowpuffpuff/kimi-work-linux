...
resolve_electron_version() {
	...
	if [ -n "${KIMI_ELECTRON_VERSION:-}" ]; then
		ELECTRON_VERSION_RESOLVED="$KIMI_ELECTRON_VERSION"
		echo "$ELECTRON_VERSION_RESOLVED"
		return
	fi
	...
	die "could not resolve Electron version (run --inspect first or set KIMI_ELECTRON_VERSION)"
}
