...
```js
module.exports = [{
  id: 'disable-auto-update',   // unique id
...
### Phases
- `main-bundle` — targets `out/main/index.js` (Kimi Work's Electron main bundle)
...
## Known candidates (not yet implemented)
- `disable-auto-update` — Kimi Work runs `electron-updater` against
  `https://kimi-img.moonshot.cn/app/upgrade/`. On an unofficial Linux build
  the updater would try to replace the app with a macOS payload; disable it.
- `linux-window-class` — pin `StartupWMClass=kimi-desktop` (already set in the
  desktop entry; a bundle patch is only needed if the app overrides it).
